...
class Estimator():
    ...
    def _build_model(self):
        ...
        conv1, w1, b1 = self.conv2d(X, 32, [8,8], [4,4], name='l1')
        # conv1 = tf.contrib.layers.conv2d(X, 32, 8, 4, activation_fn=tf.nn.relu, trainable=True)
        
        
        # second convolutional layer- input [20 * 20 * 32], filter [4 * 4], num_filters 64, stride 2
        conv2, w2, b2 = self.conv2d(conv1, 64, [4,4], [2,2], name='l2')
        #conv2 = tf.contrib.layers.conv2d(conv1, 64, 4, 2, activation_fn=tf.nn.relu)
        #tf.contrib.layers.summarize_tensor(conv2, tag='conv2')
        
        
        # third convolutional layer- input [9 * 9 * 64], filter [3 * 3], num_filters 64, stride 1
        conv3, w3, b3 = self.conv2d(conv2, 64, [3,3], [1,1], name='l3')
          
        #conv3 = tf.contrib.layers.conv2d(conv2, 64, 3, 1, activation_fn=tf.nn.relu)
        # first fully connected layer- input [7 * 7 * 64], num_filters 512
        flattened = tf.contrib.layers.flatten(conv3)
        fc1 = tf.contrib.layers.fully_connected(flattened, 512)
        # seconf fully connected layer- input 512, num_filters 4, linear
        ...
        ww1 = tf.reshape(w1, [1, 8*4,8*8,4]) # 32, 8,8,4
        ww2 = tf.reshape(w2, [1, 8*8,2*8*8,4]) # 64, 32, 4,4
        ww3 = tf.reshape(w3, [1, 64,64*3,3]) # 64, 64, 3,3
        ...
        self.summaries = tf.summary.merge([
            ...
            tf.summary.image('ww1', ww1),
            tf.summary.image('ww2', ww2),
            tf.summary.image('ww3', ww3)
...
experiment_dir = os.path.abspath("./experiments-try7/{}".format(env.spec.id))
...
with tf.Session() as sess:
    ...
    for t, stats in deep_q_learning(sess,
        ...
        np.save('Episode_stats_%s' %t , stats)
